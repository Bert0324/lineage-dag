...
import {setElementDragable} from '../../example/utils/drag'
import {getEdges} from './canvas';
...
export default class TableNode extends Node {
  ...
  draw(obj) {
    let _dom = obj.dom;
    const id = obj.name || obj.id || obj.options?.name || obj.options?.id;
    if (!_dom) {
      _dom = $('<div></div>')
    ...
    setElementDragable(node[0], setCanvas => {
    ...
    if (titleRender) {
      ...
      (this._canvas ? this._canvas._renderPromise : Promise.resolve()).then(() => {
        this._renderPromise = new Promise((resolve, reject) => {
          ...
          titleCom[0].title = titleCom[0].textContent;
        });
      });
    } else if (title) {
      titleDom.css({
      ...
      fields.forEach((_field, index) => {
        ...
        columns.forEach((_col) => {
          if (_col.render) {
            ...
            let fieldItemDom = $(`<span class="field-item" title="${_field[_col.key]}">${_field[_col.key]}</span>`);
            fieldItemDom.css('width', (_col.width || this.COLUMN_WIDTH) + 'px');
            fieldDom.append(fieldItemDom);
          }
          if (_col.primaryKey) {
