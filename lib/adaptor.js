...
export let transformInitData = (data) => {
  ...
  let result = {
    ...
    edges: relations.map((item) => {
      return {
        relationColor: item.relationColor,
        id:
          item.id ||
          `${item.srcTableId}-${item.tgtTableId}-${item.srcTableColName}-${item.tgtTableColName}`,
        type: "endpoint",
        ...
        source:
          item.srcTableColName !== undefined && item.srcTableColName !== null
            ? item.srcTableColName
            : item.srcTableId + "-right",
        target:
          item.tgtTableColName !== undefined && item.tgtTableColName !== null
            ? item.tgtTableColName
            : item.tgtTableId + "-left",
        _isNodeEdge:
          (item.srcTableColName === undefined ||
            item.srcTableColName === null) &&
          (item.tgtTableColName === undefined || item.tgtTableColName === null),
        Class: Edge,
      };
    }),
...
export let transformEdges = (nodes, edges) => {
  ...
  edges.forEach((item) => {
    let existObj =
      edgesObj[
        `${item.sourceNode}-${item.source}-${item.targetNode}-${item.target}`
      ];
...
export let diffPropsData = (newData, oldData) => {
  ...
  const isCollapseNodesChange = (a, b) =>
    a.id === b.id && a.isCollapse === b.isCollapse;
  let collapseNodes = _.differenceWith(
    newData.nodes,
    oldData.nodes,
    isCollapseNodesChange
  );
